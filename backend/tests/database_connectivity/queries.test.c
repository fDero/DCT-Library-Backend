...
#define ASSERT_NOT_NULLPTR(a_pointer) ASSERT_NE(a_pointer, nullptr)

#define EXPECT_ACCOUNT_EQUAL(actual_account, expected_account)                \
    do {                                                                      \
        EXPECT_EQ((actual_account)->account_id,                               \
...
#define EXPECT_BOOK_EQUAL(actual_book, expected_book)                          \
    do {                                                                       \
        ...
        EXPECT_TRUE(timestamp_equal(&(actual_book)->release_date,              \
                                    &(expected_book)->release_date));          \
...
#define EXPECT_LOAN_EQUAL(actual_loan, expected_loan)                      \
    do {                                                                   \
        ...
        EXPECT_TRUE(timestamp_equal(&(actual_loan)->starting_time,         \
                                    &(expected_loan)->starting_time));     \
        EXPECT_TRUE(timestamp_equal(&(actual_loan)->ending_time,           \
                                    &(expected_loan)->ending_time));       \
        EXPECT_EQ((actual_loan)->account_id, (expected_loan)->account_id); \
        EXPECT_EQ((actual_loan)->book_id, (expected_loan)->book_id);       \
    } while (0)

bool contains_account(const account_array_t* account_array,
...
bool contains_book(const book_array_t* book_array, const book_t* book) {
    ...
    for (size_t i = 0; i < book_array->size; ++i) {
        ...
        if (book->book_id == current_book->book_id &&
            ...
            timestamp_equal(&(book)->release_date,
                            &(current_book)->release_date) &&
...
bool contains_loan(const loan_array_t* loan_array, const loan_t* loan) {
    ...
    for (size_t i = 0; i < loan_array->size; ++i) {
        ...
        if (loan->loan_id == current_loan->loan_id &&
            timestamp_equal(&(loan)->starting_time,
                            &(current_loan)->starting_time) &&
            timestamp_equal(&(loan)->ending_time,
                            &(current_loan)->ending_time) &&
...
TEST_F(Database, get_books_by_author_1) {
    book_array_t* books =
        get_books_by_author(conn, "John Ronald Reuel Tolkien");
    ASSERT_NOT_NULLPTR(books);
    EXPECT_EQ(books->size, 1);
...
TEST_F(Database, get_books_by_publisher_1) {
    book_array_t* books = get_books_by_publisher(
        conn, "George Allen and Unwin (UK) Houghton Mifflin (US)");
    ASSERT_NOT_NULLPTR(books);
    EXPECT_EQ(books->size, 1);
    EXPECT_TRUE(contains_book(books, db_book_5));
    if (books != NULL) book_array_destroy(books);
...
TEST_F(Database, get_books_by_genres_3) {
    book_array_t* books =
        get_books_by_genres(conn, "Young adult literature,Education");
    EXPECT_NULLPTR(books);
    if (books != NULL) book_array_destroy(books);
}
...
TEST_F(Database, get_books_by_data_match_1) {
    book_array_t* books = get_books_by_data_match(
        conn, NULL, "The Hunger Games", "Suzanne Collins", "Scholastic Press",
        "Adventure fiction,Science fiction", "2008-09-14 00:00:00", 100);
    ASSERT_NOT_NULLPTR(books);
    EXPECT_EQ(books->size, 1);
...
TEST_F(Database, get_books_by_data_match_2) {
    book_array_t* books = get_books_by_data_match(
        conn, NULL, "Harry Potter", NULL, "The Russian Messenger", NULL, NULL, 100);
    EXPECT_NULLPTR(books);
    if (books != NULL) book_array_destroy(books);
}
...
TEST_F(Database, get_books_by_data_match_3) {
    book_array_t* books =
        get_books_by_data_match(conn, NULL , NULL, NULL, NULL, NULL, NULL, 100);
    ASSERT_NOT_NULLPTR(books);
    EXPECT_EQ(books->size, 6);
    if (books != NULL) book_array_destroy(books);
}
