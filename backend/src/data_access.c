...
account_array_t perform_account_array_query(conn_t* connection, const char* query_string)
{
	...
	assert(col_count == 5);
	account_array_t output_array;
	account_array_init(&output_array, row_count);
	for (int row = 0; row < row_count; row++)
	{
		extract_account(resultset, row, output_array.array_storage + row);
	}
	PQclear(resultset);
	return output_array;
}
...
book_array_t perform_book_array_query(conn_t* connection, const char* query_string)
{
	...
	int col_count = PQnfields(resultset);
	assert(col_count == 7);
	book_array_t output_array;
	book_array_init(&output_array, row_count);
	for (int row = 0; row < row_count; row++)
	{
		extract_book(resultset, row, output_array.array_storage + row);
	}
	PQclear(resultset);
	return output_array;
}
...
loan_array_t perform_loan_array_query(conn_t* connection, const char* query_string)
{
	...
	int col_count = PQnfields(resultset);
	assert(col_count == 5);
	loan_array_t output_array;
	loan_array_init(&output_array, row_count);
	for (int row = 0; row < row_count; row++)
	{
		extract_loan(resultset, row, output_array.array_storage + row);
	}
	PQclear(resultset);
	return output_array;
}
